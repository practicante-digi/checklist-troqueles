...
function setHeaderDate() {
    ...
    h = h % 12 || 12;
    el.innerHTML = `<span class='text-sm font-medium text_primary_color'>${date}</span>
                        <span class='ml-4 text-lg font-semibold text_primary_color'>${h}:${m}:${s} ${ampm}</span>`;
    }
    update();
...
const fpConfig = {
    ...
    altInput: true,
    altFormat: 'd \\de M Y',
    disableMobile: true, // Mantiene la UI unificada en tablets
    onOpen: function() {
        isDatepickerOpen = true;
    },
    onClose: function() {
        // Le damos un pequeño respiro (150ms) para que el clic termine de procesarse
        // antes de decirle al sistema que el calendario se cerró.
        setTimeout(() => { isDatepickerOpen = false; }, 150);
    }
};
...
// === Lógica de Dropdowns de Filtro ===

function configurarEventosFiltros() {
    // 1. Evento para abrir/cerrar los popups al hacer clic en el encabezado
    ...
    // 2. Evento para los botones "Limpiar"
    document.querySelectorAll('.btn-clear').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.preventDefault();
            const popup = e.target.closest('.filter-popup');
            
            // Limpiamos según el ID del popup en el que estamos
    ...
    // 3. Evento para los botones "Aplicar"
...
// Detector inteligente de clics (Permite que Choices.js funcione)
document.addEventListener('click', (event) => {
    // MAGIA: Si el calendario de fecha está abierto o se acaba de cerrar, ignoramos el clic
...
function limpiarFiltro(tipo) {
    // Restablecemos el valor a vacío (asegúrate de tener el <option value="">Todos</option> en tu HTML)
    if (tipo === 'troquel' && choicesTroquel) {
        choicesTroquel.setChoiceByValue('');
    }
    if (tipo === 'tecnico' && choicesTecnico) {
...
function aplicarFiltros() {
    // Cerramos el menú para darle feedback al usuario y ejecutamos la búsqueda
    cerrarTodosLosDropdowns();
    buscarHistorial(1);
}
...
async function buscarHistorial(page = 1) {
    ...
    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color:#ef4444; padding:2rem;">Error al cargar los datos. Intenta de nuevo.</td></tr>`;
    }
}
...
async function verDetalle(id) {
    ...
    <div style="text-align:center; padding:2rem;">
        <div style="width:2rem; height:2rem; border:3px solid #f3f4f6; border-top-color:#ff5c35; border-radius:50%; animation:spin 0.8s linear infinite; margin:0 auto 1rem;"></div>
    ...
    const subheaderHtml = `<div class="info-grid">
        ...
        <div class="info-value">${escapeHtml(info.NombreTecnico)}</div>
        </div>
        <div class="info-item">
        <div class="info-label">Inicio</div>
        <div class="info-value">${formatFechaHora(info.Inicio)}</div>
        </div>
        <div class="info-item">
        <div class="info-label">Fin</div>
        <div class="info-value">${formatFechaHora(info.Fin)}</div>
        </div>
        <div class="info-item">
        <div class="info-label">Duración</div>
        <div class="info-value">${formatDuracion(info.DuracionMinutos)}</div>
        </div>
    </div>`;
    ...
    if (actividades.length === 0) {
        ...
        html += `<details class="tipo-section" ${openAttr}>
            <summary>
            <span>${escapeHtml(tipoLimpio)}</span>
        ...
        acts.forEach(a => {
            const estadoClass = a.Estado === 'Completado' ? 'estado-completado'
            : a.Estado === 'No Completado' ? 'estado-no-completado'
                : 'estado-no-aplica';
            ...
            <div class="actividad-info">
                <div class="actividad-nombre">${escapeHtml(a.Actividad)}</div>
            </div>
            </div>`;
        });
        ...
        if (sectionComment) {
            html += `
            <div class="section-comment-display mt-4 p-3 bg-orange-50 rounded-lg border-l-4 border-orange-500">
