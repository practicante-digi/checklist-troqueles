...
/**
 * Inicializa el QR Manager. Se llama solo la primera vez que se activa el tab.
 */
export function initQrManager() {
    ...
    function renderCards(troqueles) {
        if (troqueles.length === 0) {
            grid.innerHTML = `<div style="grid-column: 1 / -1; text-align: center; padding: 3rem;"><p style="color: #6b7280; font-weight: 500;">No se encontraron troqueles</p></div>`;
        ...
        <div class="qr-card" data-codigo="${t.codigo.toLowerCase()}" data-id="${t.idTroquel}">
            ...
            <div class="qr-actions">
                ...
                <button class="btn-action action-open btn-open" data-troquel-id="${t.idTroquel}" title="Ir al formulario">
                <svg style="width:16px;height:16px" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    searchInput.addEventListener('input', () => {
        ...
        const filtered = term ? allTroqueles.filter(t => t.codigo.toLowerCase().includes(term)) : allTroqueles;
    ...
    grid.addEventListener('click', (e) => {
        ...
        if (downloadBtn) {
            const { qr, codigo } = downloadBtn.dataset;
            if (qr && codigo) downloadQR(qr, codigo);
        }
        if (openBtn) {
            const troquelId = openBtn.dataset.troquelId;
            if (troquelId) {
                // Navega al formulario con el troquel pre-seleccionado
                document.dispatchEvent(new CustomEvent('qr:open-troquel', {
                    detail: { troquelId }
                }));
            }
        }
    ...
    btnDownloadAll.addEventListener('click', async () => {
        ...
        btnDownloadAll.innerHTML = `<div class="spinner" style="width: 16px; height: 16px; border-width: 2px; margin-right: 6px; display: inline-block; vertical-align: middle;"></div>Comprimiendo...`;
        ...
        try {
            ...
            await Promise.all(allTroqueles.map(async (t) => {
                try {
                    if (t.qr.startsWith('data:image')) {
                        imgFolder.file(`QR-${t.codigo}.png`, t.qr.split(',')[1], { base64: true });
                    } else {
                        const respuesta = await fetch(t.qr);
                        imgFolder.file(`QR-${t.codigo}.png`, await respuesta.blob());
                    }
                } catch (err) {
                    console.error(`No se pudo agregar QR de ${t.codigo}:`, err);
                }
            }));
